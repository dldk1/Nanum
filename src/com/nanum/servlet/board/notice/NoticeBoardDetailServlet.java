 ...
 */
@WebServlet("/NoticeBoardDetailServlet")
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(board_bean.getBoard_writer_idx() != login_bean.getU_idx()){
				BoardNoticeDAO.addReadCnt(board_idx);
			}
			
			// 댓글 정보를 가져온다.
			ArrayList<ReplyBean> reply_list = BoardNoticeDAO.getReplyList(board_idx);
			request.setAttribute("reply_list", reply_list);
			
			String site = "/admin/board_detail_notice.jsp";
